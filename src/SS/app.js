/**
 * Deprecated orchestrator stub.
 * This file is no longer used as the app entry; see src/main.js instead.
 * It remains as a minimal module to avoid breaking accidental imports.
    

export function initializeApp(...args) {
  if (typeof window !== 'undefined' && typeof window.initializeApp === 'function') {
    return window.initializeApp(...args);
  }
  // no-op
}
...
// Backwards-compat: some legacy code may look for these globals. Provide safe no-ops.
if (typeof window !== 'undefined') {
  window.updateDateLabels = window.updateDateLabels || function () {};
  window.isochronesToggle = window.isochronesToggle || function () {};
  window.showIsochrones = window.showIsochrones || function () {};
  window.clearIsochrones = window.clearIsochrones || function () {};
    /*
     * Deprecated: everything below is legacy code retained only for reference.
     * It is commented out to prevent parse errors. See src/main.js for the real entry.
     */
/* app.js legacy content commented out */
}
}
}
    ...
    if (newIndex !== currentIndex) {
        ...
        window.AdeckReader.displayTracksByInitTime(
            ...
            window.currentModelFilterPreference !== undefined ? 
                window.currentModelFilterPreference : false
        );
...
function showStormSelectionDialog(storms) {
    ...
    <div class="dialog-controls">
        <div class="track-toggle-container">
            ...
            <button id="toggle-model-filter" class="toggle-tracks-btn model-filter-btn" title="Toggle between all models and core guidance">All Models</button>
        </div>
    ...
    categories.forEach(category => {
        ...
        checkbox.addEventListener('change', function() {
            if (this.checked && !isBdeckTrackLoaded) {
                ...
                const showCoreOnly = window.currentModelFilterPreference !== undefined ?
                    window.currentModelFilterPreference : false;
            
                // Display the filtered tracks
                displayAdeckTracks(filteredStorms, selectedStormId, showCoreOnly, selectedCategory);
    ...
    if (toggleAllTracksBtn) {
        toggleAllTracksBtn.classList.add('adeck-global-toggle');
        toggleAllTracksBtn.addEventListener('click', function() {
            const currentlyVisible = window.adeckGlobalVisibility !== false;
            setAllModelVisibility(!currentlyVisible, { respectIndividual: true });
        });
        refreshGlobalModelVisibilityUI();
    }

    const toggleModelFilterBtn = dialog.querySelector('#toggle-model-filter');
    if (toggleModelFilterBtn) {
        const updateModelFilterButton = (coreOnly) => {
            if (coreOnly) {
                toggleModelFilterBtn.textContent = 'Core Models';
                toggleModelFilterBtn.classList.add('active');
                toggleModelFilterBtn.title = 'Showing core guidance models. Click to show all models.';
            } else {
                toggleModelFilterBtn.textContent = 'All Models';
                toggleModelFilterBtn.classList.remove('active');
                toggleModelFilterBtn.title = 'Showing all forecast models. Click to limit to core guidance.';
            }
        };

        toggleModelFilterBtn.addEventListener('click', () => {
            const currentPreference = window.currentModelFilterPreference !== undefined
                ? window.currentModelFilterPreference
                : false;
            const nextPreference = !currentPreference;
            window.currentModelFilterPreference = nextPreference;
            window.lastModelFilterPreference = nextPreference;

            const initDropdown = dialog.querySelector('.init-time-dropdown');
            const currentInit = initDropdown ? initDropdown.value : null;

            const sourceStorms = (window.adeckStorms && window.adeckStorms.length)
                ? window.adeckStorms
                : storms;
            ...
            updateModelFilterButton(nextPreference);

            showNotification(
                nextPreference ? 'Showing core guidance models' : 'Showing all forecast models',
                'info',
                1500
            );
        });

        const initialPreference = window.currentModelFilterPreference !== undefined
            ? window.currentModelFilterPreference
            : false;
        updateModelFilterButton(initialPreference);
    }
...
function refreshGlobalModelVisibilityUI() {
    const globalVisible = window.adeckGlobalVisibility !== false;
    document.querySelectorAll('.adeck-global-toggle').forEach(button => {
        if (!button) return;
        button.textContent = globalVisible ? 'Hide Models' : 'Show Models';
        button.title = globalVisible ? 'Hide all forecast model tracks' : 'Show all forecast model tracks';
        button.dataset.visible = globalVisible ? 'true' : 'false';
    });
}

function setAllModelVisibility(visible, options = {}) {
    const {
    ...
    if (updateGlobal) {
        window.adeckGlobalVisibility = visible;
    }

    const hiddenTracks = respectIndividual && window.AdeckReader && window.AdeckReader.hiddenTracks
        ? window.AdeckReader.hiddenTracks
        : {};
    const layerGroup = window.adeckLayerGroup;
    if (!layerGroup) {
        if (updateUI) {
            refreshGlobalModelVisibilityUI();
        }
    ...
    const shouldShowTrack = (trackId) => {
        if (!visible) {
            return false;
        }
        if (!respectIndividual) {
            return true;
        }
        return !hiddenTracks[trackId];
    };

    if (window.adeckLines) {
        window.adeckLines.forEach(line => {
            if (!line) return;
            const trackId = line.stormId || (line.options && line.options.stormId);
            const displayLine = shouldShowTrack(trackId);

            if (displayLine) {
                if (!layerGroup.hasLayer(line)) {
                    line.addTo(layerGroup);
                }
                if (line.setStyle) {
                    const originalOpacity = line.options.originalOpacity !== undefined
                        ? line.options.originalOpacity
                        : (line.options.opacity !== undefined ? line.options.opacity : 1);
                    line.options.originalOpacity = originalOpacity;
                    line.setStyle({
                ...
                if (line._path) {
                    line._path.style.display = '';
                }
                line._adeckHidden = false;
                if (respectIndividual && hiddenTracks[trackId]) {
                    line._adeckHiddenByUser = true;
                } else if (visible) {
                    line._adeckHiddenByUser = false;
                }
            } else {
                if (layerGroup.hasLayer(line)) {
                    layerGroup.removeLayer(line);
                }
                line._adeckHidden = true;
            }
        });
    }
    ...
    if (window.adeckMarkers) {
        window.adeckMarkers.forEach(marker => {
            if (!marker) return;
            const trackId = marker.stormId || (marker.options && marker.options.stormId);
            const displayMarker = shouldShowTrack(trackId);
            ...
            if (displayMarker) {
                ...
                if (window.AdeckReader && typeof window.AdeckReader.setMarkerVisibility === 'function') {
                    window.AdeckReader.setMarkerVisibility(marker, true);
                }
                marker._adeckHidden = false;
                if (respectIndividual && hiddenTracks[trackId]) {
                    marker._adeckHiddenByUser = true;
                } else if (visible) {
                    marker._adeckHiddenByUser = false;
                }
            } else {
                if (window.AdeckReader && typeof window.AdeckReader.setMarkerVisibility === 'function') {
                    window.AdeckReader.setMarkerVisibility(marker, false);
...
// Expose helpers for other modules and UI controls
window.setAllModelVisibility = setAllModelVisibility;
window.refreshGlobalModelVisibilityUI = refreshGlobalModelVisibilityUI;

// Add this function to handle ADECK track display
function displayAdeckTracks(storms, selectedStormId = null, defaultModelsOnly = false, categoryOverride = null) {
    ...
    if (categoryOverride !== null && categoryDropdown && categoryDropdown.value !== categoryOverride) {
        categoryDropdown.value = categoryOverride;
    }
    const selectedCategory = categoryOverride !== null
        ? categoryOverride
    ...
    processedStorms.forEach(storm => {
        const isUserHidden = window.AdeckReader && window.AdeckReader.hiddenTracks && window.AdeckReader.hiddenTracks[storm.id];
        const globallyHidden = window.adeckGlobalVisibility === false;
        ...
        if (points.length > 0) {
            ...
            trackLine.options.originalOpacity = opacity;

            if (!isUserHidden && !globallyHidden) {
                trackLine.addTo(window.adeckLayerGroup);
            } else {
                trackLine._adeckHidden = true;
                trackLine._adeckHiddenByUser = !!isUserHidden;
            }
        ...
        storm.points.forEach((point, index) => {
            ...
            marker.options.originalOpacity = opacity;

            if (!isUserHidden && !globallyHidden) {
                marker.addTo(window.adeckLayerGroup);
            } else {
                marker._adeckHidden = true;
                marker._adeckHiddenByUser = !!isUserHidden;
            }
    ...
    // Re-apply global and per-model visibility preferences now that layers exist
    setAllModelVisibility(window.adeckGlobalVisibility !== false, {
        respectIndividual: true,
        updateGlobal: false,
        updateUI: false
    });
...
function updateDateLabels() {
    ...
    if (data && data.length > 0 && markers && markers.length > 0) {
        markers.forEach((marker, index) => {
            ...
            // Get formatted date and time for CSV tracks
            let dateTimeLabel = "";
